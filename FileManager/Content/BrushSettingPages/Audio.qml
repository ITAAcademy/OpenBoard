import QtQuick 2.0
import QtGraphicalEffects 1.0
import QtQuick.Controls 1.2
Rectangle {
    id: root
    color: "transparent"
//    anchors.fill: parent
    height: parent.height
    width: parent.width
    property string cLoaderName: "loader"
    property int last_click_index : -1
    property Item p_rep
    function update()
    {
        rep.model = brushControll.getMusicStackSize();
    }

    ScrollView
    {
        id: scroll
      //  contentItem: pages
        //verticalScrollBarPolicy: Qt.ScrollBarAlwaysOn
        width: parent.width
        height: parent.height
    Column{
        id: view
        //anchors.fill: parent
       // rows: rep.model/4;
       // columns: 4; spacing: 20;

        Repeater{
            id: rep
            Component.onCompleted: {
                p_rep = rep
            }

            model: brushControll.getMusicStackSize()
            function abortBorders()
            {
                for (var i=0;  i< rep.count; i++)
                {
                    rep.itemAt(i).border.color = "#DDDDDD"
                    rep.itemAt(i).border.width = 0
                }
            }
            delegate:
            Rectangle{
                id: brush
                property int mIndex : index
                property string filename : brushControll.getMusicStackAt(index)
                MouseArea{
                    id: img_MA
                    anchors.fill: parent
                    hoverEnabled: true
                    onClicked:
                    {
                        finishFileName = filename
                        rep.abortBorders()
                        border.color = "aqua"
                        border.width = 6
                        root.update();
                        if (root.last_click_index == brush.mIndex  && double_click_timer.running)
                        {

                            brushControll.setFilePathName(finishFileName)
                             brushControll.hide()

                           root.last_click_index = -1
                        }
                        else
                        {
                            root.last_click_index = brush.mIndex
                            double_click_timer.running = true
                        }
                    }
                    onEntered: {
                        if ( brush.border.width == 0)
                        brush.border.width = 4;
                    }
                    onExited: {
                         if ( brush.border.width == 4)
                        brush.border.width = 0;
                    }
                }

                height: 50
                width: root.width - 20
                color: "transparent"
                border.color: "#DDDDDD"
                border.width: 0
                radius: 10
                    Image{
                    id: img

                    height: parent.height
                    width: height
                    antialiasing: true
                    source: "qrc:/iphone_toolbar_icons/Audio.png"


                        Text{
                            text : brushControll.getMusicStackOnlyFilenameAt(index)
                            //width: brush.width -img.height
                            x: img.height
                            y: (img.height - height)/2
                            color: "white";
                            font { pointSize: 9}
                            //height: img.height
                            //anchors.left: img.right
                        }
                    }

                }
            }
    }

    }
}

